...
class Bus {
	...
	public void returnReserveNum() {
		StringBuilder reservenum = new StringBuilder();

		if(this.startingPoint.equals("서울")) {
			reservenum.append("S");
		}
		else if(this.startingPoint.equals("부산")) {
			reservenum.append("B");}
		else if(this.startingPoint.equals("대구")) {
			reservenum.append("D");
		}
		else if(this.startingPoint.equals("인천")) {
			reservenum.append("I"); }
		else if(this.startingPoint.equals("광주")) {
			reservenum.append("G");
		}

		if(this.destination.equals("서울")) {
			reservenum.append("S");
		}
		else if(this.destination.equals("부산")) {
			reservenum.append("B");
		}
		else if(this.destination.equals("대구")) {
			reservenum.append("D");
		}
		else if(this.destination.equals("인천")) {
			reservenum.append("I");
		}
		else if(this.destination.equals("광주")) {
			reservenum.append("G");
		}

		reservenum.append(Character.toString(startTime.charAt(0)));
		reservenum.append(Character.toString(startTime.charAt(1)));
...
public class Main {
	...
	public static String[] time = {"07:00","10:00","13:00","16:00","19:00","22:00"};
	...
	public static void saveBusData(String goDate) {
		// TODO Auto-generated method stub
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				if(i!=j) {
					for (int k = 0; k < time.length; k++) {
						Bus b = new Bus(depPoint[i],arrPoint[j],time[k]);
						b.setStartDate(goDate);
						bus.add(b);
		...
		while(true) {
			while(true) {
				...
				if(name.length()<=1||name.length()>=6) {
					System.out.println("이름은 2-5글자의 한글 입력만 허용합니다");
					continue;
				}
	...
	public static void startMenu(Member member) throws Exception {
		while(true) {
			...
			System.out.println("1. 예매    2. 정기권 구매    3. 마이페이지    4. 종료");
	...
	public static int checkReservation(Member member) throws Exception {
		// TODO Auto-generated method stub
		while(true) {
	...
	public static void reviseMember(Member member) throws Exception {
		...
		char selectnum;
		String select;
		String newname, newphoneNum, newid, newpwd, newq, newa;
		String temppwd;
		while(true) {
	...
	public static void reserve(Member member) throws Exception {
		...
		while(true) {
			...
			while(nowTime==null) {
				System.out.print("현재 시간을 다시 입력하세요:");
				nowTime = br.readLine();
				nowTime = checkTime(nowTime);
			} 
			member.time = nowTime;
			...
			while(true) {
				goDate = checkDate(goDate);
				if(goDate != null) {
					if(Integer.parseInt(nowDate) <= Integer.parseInt(goDate)) {
						break;
					}
					else {
						System.out.println("출발 날짜는 현재 날짜보다 빠를 수 없습니다.");
					}
				}
				System.out.print("출발 날짜를 입력하세요:");
				goDate = br.readLine();
					...
					for (int j = 0; j < 5; j++) {
						if(arr.equals(arrPoint[j])) {
							...
							if(member.ticketCnt>0) {
								Bus bookedBus = bookSeatTicket(nowDate,member,dep,arr,depTime,seat,goDate);
								if(bookedBus!=null) {
									bookedBus.setNum(seat);
									bookedBus.startDate = goDate;
									bookedBus.destination = arr;
									bookedBus.returnReserveNum();
									member.b.add(bookedBus);
									return;
								}

							}else if(member.ticketCnt==0){
								Bus bookedBus = bookSeat(dep,arr,depTime,seat,goDate);
								if(bookedBus!=null) {
									bookedBus.setNum(seat);
									bookedBus.startDate = goDate;
									bookedBus.destination = arr;
	...
	private static Bus bookSeatTicket(String nowDate, Member member, String dep, String arr, String depTime, int seat,String goDate) throws Exception {
		// TODO Auto-generated method stub
		for (int i = 0; i < bus.size(); i++) {
			if(bus.get(i).startingPoint.equals(dep)&&bus.get(i).destination.equals(arr)) {
				//bus.get(i).startTime = checkTime(bus.get(i).startTime);

				if(bus.get(i).startDate.equals(goDate)) {
					...
					if(bus.get(i).startTime.equals(depTime)) {
						...
						if(bus.get(i).people+seat<=10) {
							...
							System.out.print("원하시는 좌석을 입력해주세요:(좌석이 1개일 때:01)(좌석이 3개일 때:01 02 10)");
	...
	public static Bus bookSeat(String dep, String arr, String depTime, int seat,String goDate) throws Exception {
		// TODO Auto-generated method stub
		for (int i = 0; i < bus.size(); i++) {
			if(bus.get(i).startingPoint.equals(dep)&&bus.get(i).destination.equals(arr)) {
				if(bus.get(i).startDate.equals(goDate)) {
		...
		while(true) {
			...
			if(phoneNum.charAt(3)=='-') {
				...
				if(phoneNum.length()!=11) {
					System.out.println("전화번호는 11개의 숫자로 이루어져있습니다.");
					continue;
	...
	public static int pwdCheck(String pwd) {
		...
		for (int i = 0; i < pwd.length(); i++) {
			boolean result = Character.toString(pwd.charAt(i)).matches("[0-9|a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힝]*");
